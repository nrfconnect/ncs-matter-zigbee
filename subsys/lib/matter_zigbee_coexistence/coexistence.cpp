...
LOG_MODULE_REGISTER(matter_zigbee_coexistence, CONFIG_MATTER_ZIGBEE_COEXISTENCE_LOG_LEVEL);
...
void matter_event_handler(const chip::DeviceLayer::ChipDeviceEvent *event, intptr_t arg)
{
	...
	case chip::DeviceLayer::DeviceEventType::kServerReady:
		...
		break;
#endif /* CONFIG_MATTER_ZIGBEE_COEXISTENCE_BT_ADV_WHILE_ZIGBEE */
...
extern "C" void matter_zigbee_coexistence_signal_matter_board_init(void)
{
	matter_board_init_signal();
}

extern "C" void matter_zigbee_coexistence_on_server_started(void)
{
#if defined(CONFIG_OPENTHREAD)
...
extern "C" void matter_zigbee_coexistence_pre_server_init(void)
{
#if defined(CONFIG_OPENTHREAD)
								...
								uint32_t switch_button)
{
	...
	if (!press_active && pressed) {
		...
		k_work_reschedule(&protocol_switch_work,
				  K_SECONDS(CONFIG_MATTER_ZIGBEE_COEXISTENCE_SWITCH_BUTTON_PRESS_TIME_SECONDS));
		return false;
	}

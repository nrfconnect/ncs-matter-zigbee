...
Shared memory requirement data for Matter and Zigbee coexistence documentation.

Board partition layouts and usage values live in docs/data/memory/<board>.yaml.
Sample variants are discovered from Twister sample.yaml files under samples/.
"""
...
from memory_samples import build_board_samples
...
def load_board_data(board: str) -> dict[str, Any]:
    ...
    usage_by_test = data.pop("usage_by_test", {})
    data["samples"] = build_board_samples(board, usage_by_test)
    return data

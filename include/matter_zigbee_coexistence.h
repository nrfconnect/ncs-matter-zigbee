...
#ifndef MATTER_ZIGBEE_COEXISTENCE_H_
#define MATTER_ZIGBEE_COEXISTENCE_H_

/** @file
 * @defgroup matter_zigbee_coexistence Matter + Zigbee coexistence runtime
 * @{
 *
 ...
 * persistent protocol state (see @ref matter_zigbee_protocol_state) up to date
 * across reboots and factory resets.
 *
 * Samples integrate the module by populating @ref matter_zigbee_coexistence_callbacks
 * and calling @ref matter_zigbee_coexistence_run from @c main().
 */
 ...
 * @retval 0 on success
 */
int matter_zigbee_coexistence_run(const struct matter_zigbee_coexistence_callbacks *cb);
 ...
 *
 * - When @kconfig{CONFIG_MATTER_ZIGBEE_COEXISTENCE_BT_ADV_WHILE_ZIGBEE} is
 ...
 * crashing.  @ref matter_zigbee_coexistence_on_server_started restores the
 * dispatcher to Zigbee once @c StartServer() returns.
 *
 * This function is a no-op when the device boots in Matter mode.
 */
void matter_zigbee_coexistence_pre_server_init(void);
 ...
 * when @kconfig{CONFIG_MATTER_ZIGBEE_COEXISTENCE_SWITCH_BUTTON_PRESS_TIME_SECONDS}
...
#endif /* MATTER_ZIGBEE_COEXISTENCE_H_ */
